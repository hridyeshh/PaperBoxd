...
- **Web:** We use a session cookie (httpOnly, so it cannot be read by JavaScript) for authentication. We also set a second, non-httpOnly cookie holding basic display information about your account (such as your username and avatar) so the interface can render it without an extra request; it contains no credentials or session token. We do not use third-party advertising or analytics cookies.
...
- **Manual export/portability:** We do not yet offer self-serve data export. To request a copy of your data, email us at hridyesh@paperboxd.in and we will provide it within 30 days.
...
To exercise any of these rights, contact us at hridyesh@paperboxd.in.
...
Questions about this Policy or your data: hridyesh@paperboxd.in
