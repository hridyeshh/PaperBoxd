          ...
          {lists.length > 0 ? (
            lists.map((l) => (
              <button
                key={l.id}
                type="button"
                onClick={() => user?.username && router.push(`/u/${user.username}/lists/${l.id}`)}
                className={cn(
                  "flex w-full items-center justify-between px-2.5 py-[7px] rounded-lg text-[13px] text-muted-foreground hover:bg-muted/60 hover:text-foreground transition-colors text-left cursor-pointer",
