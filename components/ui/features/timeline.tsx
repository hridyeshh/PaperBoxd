...
const TimelineItem = ({ item, index, isEven, dotOffset, heightTransform }: TimelineItemProps) => {
  ...
  const outerGlowScale = useTransform(beamProgress, [0, 1], [1, 1.5]);
  const outerGlowOpacity = useTransform(beamProgress, [0, 0.5, 1], [0, 0.8, 0]);
  const innerDotScale = useTransform(beamProgress, [0, 1], [1, 1.1]);
  const colorOverlayOpacity = useTransform(beamProgress, [0, 1], [0, 1]);
  const pulsingGlowScale = useTransform(beamProgress, [0, 0.5, 1], [0.8, 1.2, 1]);
  const pulsingGlowOpacity = useTransform(beamProgress, [0, 0.5, 1], [0, 0.8, 0.6]);
  const contentOpacityEven = useTransform(beamProgress, [0, 0.5], [0.5, 1]);
  const contentOpacityOdd = useTransform(beamProgress, [0, 0.5], [0.6, 1]);
  const titleFilter = useTransform(beamProgress, [0, 1], ['brightness(0.8)', 'brightness(1.2)']);
