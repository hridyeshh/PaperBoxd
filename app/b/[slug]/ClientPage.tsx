...
export default function BookDetailPage() {
    ...
    const checkBookStatus = async () => {
      try {
        ...
        const [likedRes, bookshelfRes, tbrRes, progressRes] = await Promise.all([
          fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/books?type=liked`),
          fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/books?type=bookshelf`),
          fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/books?type=tbr`),
          bookId ? fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/reading-progress?bookId=${encodeURIComponent(bookId as string)}`) : null,
        ]);
  ...
  React.useEffect(() => {
    if (isShareOpen && session?.user?.username) {
      ...
      fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(session.user.username)}/following`)
        .then((res) => {
  ...
  React.useEffect(() => {
    ...
    const fetchCarousels = async () => {
      try {
        ...
        const promises = [
          fetch(`${API_BASE_URL}/api/recommendations/similar/${bookId}?limit=20`)
        ...
        if (primaryAuthor) {
          promises.push(
            fetch(`${API_BASE_URL}/api/books/by-author?author=${encodeURIComponent(primaryAuthor)}&excludeBookId=${bookId}&limit=20`)
              .then(res => res.ok ? res.json() : { books: [] })
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(session.user.username)}/books`, {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            type: "liked",
