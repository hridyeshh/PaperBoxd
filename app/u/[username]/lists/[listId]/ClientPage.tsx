...
export default function ListDetailPage() {
  ...
  const handleEditList = async () => {
    ...
    try {
      setIsUpdating(true);
      const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/lists/${listId}`, {
        method: "PATCH",
  ...
  React.useEffect(() => {
    ...
    const checkIfSaved = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(sessionUsername)}/lists`);
        if (!isMounted) return;
  ...
  const fetchAllowedUsers = React.useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/lists/${listId}/access`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSendToList = async (targetUsername: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/lists/${listId}/share`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSaveList = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(username)}/lists/${listId}/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
  ...
  const handleRemoveSavedList = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/users/${encodeURIComponent(session.user.username)}/lists/${savedListId}/save`, {
        method: "DELETE",
      });
