...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get username from auth user or fetch from database
    let username: string | undefined = authUser.username;
    if (!username) {
      const user = await User.findById(authUser.id).select('username').lean();
