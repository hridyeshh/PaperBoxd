...
export async function GET(
  ...
  try {
    ...
    if (!authUser?.id) {
      console.log(`[AUTH ERROR] No auth user for ${username}. AuthUser:`, authUser);
      return NextResponse.json(
    ...
    console.log(`[AUTH] User ${authUser.email} requesting activities for ${username}`);
    ...
    if (!currentUser) {
      console.log(`[AUTH ERROR] User not found in DB for id: ${authUser.id}`);
